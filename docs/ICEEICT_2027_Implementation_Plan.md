...
**Superseded by the paired seed test (Cell 12d, `table13`).** At three seeds this comparison was unresolvable; at 20 paired seeds it resolves into a **crossover**, not a win. Anchoring is better at h=1 (+4.86 skill points, d_z = 3.53), indistinguishable at h=2 and h=4, and *worse* at h=3 (−5.54, d_z = −1.50). The paper must not be framed as "our reparameterisation wins". The honest claim is that the better parameterisation is horizon-dependent, which is a sharper and more defensible result than a win would have been.

### Revised recommendation

1. **Lead with calibration — but not on priority.** Cleanest and most operationally serious, and it *strengthened* at district resolution. It is **not** the first coverage validation in dengue forecasting: see `RELATED_WORK_NOTES.md`, which cites prior work doing exactly this. Any sentence claiming otherwise must not ship. The defensible contribution is the *conditional* result — marginal coverage hides a burden gradient, and that gradient survives calibration. Current numbers: prospective 90% intervals reach 0.883 / 0.887 / 0.893 against nominal 0.90, and the burden gap is 0.072 under split conformal (z = 7.17) versus −0.004 group-conditional (p = 0.73). Quote from `results/`, never from this file.

   **The mechanism survives its confounds (`table5d`).** Coverage falls with district burden (ρ = −0.610). The obvious objection is that burden proxies for district size, so both are tested: population count is not associated at all (ρ = −0.126, p = 0.32), and population *density* is associated marginally (ρ = −0.309, p = 0.013) but that association **vanishes** once burden is controlled (ρ = −0.116, p = 0.37). Burden controlling for density barely moves (ρ = −0.561, p = 2×10⁻⁶). So the gradient is in disease burden, not in how big or how crowded a district is — which is the version of the claim the paper can defend under review.
...
src/                  never built; the eight-module split below was planned and not
                      taken. The pipeline is one file: notebooks/ml_notebook3.py
artifacts/            results/ + figures/ from a LOCAL run (git-ignored scratch)
results/  figures/    the committed canonical run, produced on Kaggle
scripts/              build_notebook.py: regenerates the .ipynb from the .py
...
- **One seeded command reproduces everything.** `python notebooks/ml_notebook3.py` regenerates
  every table and figure. There is no `run_all.py`; the eight-module `src/` split in §5 was
  planned and never built, and the single notebook is what actually runs.
...
*Pilot figures in §2 were computed from `data/raw/district_panel/` and `data/raw/divisional/` under the protocol stated there. Indicative, not final — regenerate via `python notebooks/ml_notebook3.py` and quote from `results/`, not from this file.*
